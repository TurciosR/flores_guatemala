...
$columns = array(
  ...
  array('db' => '`cliente`.`telefono1`', 'dt' => 4, 'field' => 'telefono1'),
  array('db' => '`credito`.`total`', 'dt' => 5, 'field' => 'total'),
  array('db' => '`credito`.`abono`', 'dt' => 6, 'field' => 'abono'),
  array('db' => '`credito`.`saldo`', 'dt' => 7, 'field' => 'saldo'),
  array(
    'db' => '`credito`.`id_credito`', 'dt' => 8, 'formatter' => function ($id_credito, $row) {
      $txt_estado = estado_label($id_credito);
      return $txt_estado;
    },
    'field' => 'id_credito'
  ),
  array(
    'db' => '`credito`.`id_credito`', 'dt' => 9, 'formatter' => function ($id_credito, $row) {
      ...
      $link = permission_usr($id_user, $filename);
      if ($link != 'NOT' || $admin == '1') {
        if (verificar_estado($id_credito) == 1) {
          $sql_factura_ref = _fetch_array(_query("SELECT * FROM factura WHERE id_factura = $id_factura"));
          $n_ref = $sql_factura_ref['numero_ref'];
          $menudrop .= "<li><a href='$filename?n_ref=$n_ref' ><i class='fa fa-dollar'></i> Facturar</a></li>";
        }
      }
      $menudrop .= "</ul>
        </div>";
...
function estado_label($id_credito)
{
  $credito = $id_credito;
...
function verificar_estado($id_credito)
{
  $credito = $id_credito;
  $sql = _fetch_array(_query("SELECT credito.total,credito.abono,credito.saldo, credito.id_factura FROM credito WHERE credito.id_credito=$credito"));
  $abono = $sql['abono'];
  $saldo = $sql['saldo'];
  $total = $sql['total'];
  $id_factura = $sql['id_factura'];

  $sql_factura = _fetch_array(_query("SELECT * FROM factura WHERE id_factura = $id_factura"));
  $factura_finalizada =  $sql_factura['finalizada'];

  $txt_estado = "";
  if (($saldo > 0 && $abono < $total) || $factura_finalizada == 1) {
    # code...
